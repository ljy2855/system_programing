...
#define DISCONNECT(ptr) CONNECT(PRED(ptr), SUCC(ptr))
#define INSERT_LIST(new, target)     \
    CONNECT(new, SUCC(target)); \
    CONNECT(target, new);
#define ERASE(ptr) CONNECT(PRED(ptr), SUCC(ptr))

#define LEFT_CHLD(ptr) PRED(ptr)
#define RIGHT_CHLF(ptr) SUCC(ptr)

#define PARENT_PTR(ptr) ((char *)(ptr) + (2 * WSIZE))
#define PARENT(ptr) (char *)(free_head + GET(PARENT_PTR(ptr)))
...
int mm_init(void)
{
    ...
    PUT(heap_head + WSIZE, 0); /* free list head p*/
...
#ifdef DEBUG
    // printf("[free start]  allocated = %d, addr = %p, size = %u, epli : %p\n\n",
    //        GET_ALLOC(HDRP(ptr)), ptr, GET_SIZE(HDRP(ptr)),heap_tail);
